(() => {
  ...
  async function handleCreateFile() {
    ...
    try {
      ...
      setLoading(true);
      const response = await jsonFetch('/api/file', {
        method: 'POST',
        body: { path: state.currentPath, name },
      });
      createdPath = response?.path || null;
      await loadDirectory(state.currentPath, { silent: true });
      if (createdPath) {
        state.selected = new Set([createdPath]);
        renderItems();
        updateActionButtons();
      }
    ...
    if (createdPath) {
      await openFileEditor(createdPath);
    }
  }
  ...
  function bindEvents() {
    ...
    elements.createFileBtn.addEventListener('click', handleCreateFile);
