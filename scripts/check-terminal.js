...
const TERMINAL_PORT = FILE_PORT;
const TERMINAL_BASE_PATH = '/terminal';
const ORIGIN = `http://127.0.0.1:${FILE_PORT}`;
const TERMINAL_BASE_URL = `${ORIGIN}${TERMINAL_BASE_PATH}`;
const TERMINAL_API_BASE = `${TERMINAL_BASE_URL}/api`;
const WS_URL = `ws://127.0.0.1:${TERMINAL_PORT}${TERMINAL_BASE_PATH}/ws`;
...
async function fetchSessions() {
  const response = await fetch(`${TERMINAL_API_BASE}/terminals`);
...
async function closeSession(sessionId) {
  const response = await fetch(`${TERMINAL_API_BASE}/terminals/${encodeURIComponent(sessionId)}/close`, {
    method: 'POST',
  });
  if (!response.ok) {
    const body = await response.text();
...
async function fetchHistory(sessionId) {
  const response = await fetch(
    `${TERMINAL_API_BASE}/terminals/${encodeURIComponent(sessionId)}/history`
  );
